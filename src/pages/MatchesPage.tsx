...
export default function MatchesPage() {
    ...
    /** ✅ USER: auto-refresh (sem botões) */
    useEffect(() => {
        if (!readOnlyUser) return;
        if (!groupId) return;

        const t = window.setInterval(() => {
            // atualiza lista/partida atual para acompanhar status
            loadList();
        }, 15000);
    ...
    async function acceptInvite(playerId: string) {
        ...
        // ✅ user só no próprio playerId
        if (!canUserActOnPlayer(admin, activePlayerId, playerId)) return;
            ...
            <div className="card p-4">
                ...
                <div className="mt-3 grid gap-2">
                    {items.map((p) => {
                        ...
                        // ✅ user só enxerga botões no próprio item
    ...
    /** ========= Read-only cards for USER ========= */

    const readOnlyTeamsView = useMemo(() => {
        if (!current) return null;
    ...
    const readOnlyPostGameView = useMemo(() => {
        ...
        const scoreText =
            current?.teamAGoals == null || current?.teamBGoals == null ? "—" : `${current.teamAGoals} x ${current.teamBGoals}`;
        ...
        return (
            <div className="card p-4">
                <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="pill">{current?.computedMvp?.playerName ?? "MVP —"}</span>
                </div>
                ...
                <div className="mt-4 grid md:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-2 text-lg font-bold text-slate-900">{current?.computedMvp?.playerName ?? "—"}</div>
                        <div className="text-xs text-slate-500 mt-1">Parciais: {(current?.voteCounts ?? []).length}</div>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {readOnlyUser ? (
                            <>
                                <div className="card p-4">
                                    ...
                                    {acceptedOverLimit ? (
                                        <div className="mt-3 text-sm text-red-700">
                                            Passou do limite de aceitos: <b>{accepted.length}</b> / <b>{maxPlayers}</b>. O admin precisa ajustar para avançar.
                                        </div>
                                    ) : null}

                                    <div className="mt-3 text-xs text-slate-500">
                                        Modo usuário: você só consegue <b>aceitar/recusar o seu próprio convite</b> quando estiver em “Aceitação”. Todo o restante é controlado pelo admin.
                                    </div>
                                </div>
                                ...
                                {stepKey === "teams" ? readOnlyTeamsView : null}

                                {stepKey === "playing" ? (
                                    <div className="card p-4">
                                        <div className="font-semibold">Jogo</div>
                                        <div className="text-xs text-slate-500">Partida iniciada (visualização).</div>
                                    </div>
                                ) : null}
                                ...
                                {stepKey === "ended" ? (
                                    <div className="card p-4">
                                        ...
                                        <div className="text-xs text-slate-500">Partida encerrada. Aguardando admin ir para Pós-jogo.</div>
                                    </div>
                                ) : null}

                                {stepKey === "post" ? readOnlyPostGameView : null}

                                {stepKey === "done" ? (
                                    <div className="card p-4">
                                        <div className="font-semibold">Finalizada</div>
                                        <div className="text-xs text-slate-500">Partida encerrada e contabilizada (visualização).</div>
                                ...
                                {/* Histórico (somente visual, sem clique/botão) */}
                                <div className="card p-4">
                                    <div className="font-semibold">Histórico</div>
                                    <div className="muted">Somente visualização (sem ações).</div>
                                    <div className="mt-3 grid gap-2">
                                        {matches.map((m) => {
                                            ...
                                            return (
                                                <div key={id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2">
                                                    <span className="text-sm text-slate-800">
                                                        {m.playedAt ? new Date(m.playedAt).toLocaleString() : "Partida"} •{" "}
                                                        {m.statusName ?? m.status}
                                                    </span>
                                                    <span className="pill">{id === currentMatchId ? "Ativa" : "—"}</span>
                                                </div>
                                            );
                                        })}
                            ...
                            <>
                                {/* ===== ADMIN MODE (seu fluxo completo original) ===== */}
                                {showCreate ? (
                                                ...
                                                <div className="card p-4">
                                                    ...
                                                    {stepKey === "teams" ? (
                                                        <div className="card p-4">
                                                            ...
                                                            {/* ✅ AQUI: CARD VISUAL DOS TIMES (SÓ QUANDO JÁ ESTÃO SETADOS) */}
                                                            {teamsAlreadyAssigned ? (
                                                                <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                                                                    <div className="flex items-center justify-between gap-3">
                                                                        <div>
                                                                            ...
                                                                            <div className="text-xs text-slate-500">
                                                                                Use essa lista para decidir quem trocar (é a mesma base dos combos do swap).
                                                                            </div>
                                                                        </div>
