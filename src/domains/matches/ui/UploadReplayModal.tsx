...
import type { ReplayClipDto, ReplayEventType } from "../matchTypes";

type EventType = Extract<ReplayEventType, "Gol" | "Jogada">;
...
function eventLabel(type: EventType): string {
    return type === "Gol" ? "Gols" : "Jogadas";
}

function eventHint(type: EventType): string {
    return type === "Gol" ? "videos de gol" : "videos de jogada";
}

function eventAccent(type: EventType): string {
    return type === "Gol" ? "bg-emerald-500" : "bg-blue-500";
}
    ...
    return (
        <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{eventLabel(eventType)}</span>
            </div>
            ...
            >
                ...
                <p className="text-xs font-medium text-slate-500 dark:text-slate-400 text-center">
                    Clique ou arraste<br />{eventHint(eventType)}
                </p>
            </div>
            <input
                ref={inputRef}
...
export function UploadReplayModal({ groupId, matchId, onClose, onUploaded }: Props) {
    ...
    const addFiles = useCallback((files: File[], eventType: EventType) => {
        ...
        setEntries((prev) => {
            ...
            return [...prev, ...newEntries].sort((a, b) => {
                const byType = eventOrder[a.eventType] - eventOrder[b.eventType];
                return byType !== 0 ? byType : naturalSort(a.file, b.file);
            });
        });
    }, []);
    ...
    const sections = [
        { label: "Gols", entries: gols, accent: eventAccent("Gol") },
        { label: "Jogadas", entries: jogadas, accent: eventAccent("Jogada") },
    ];
            ...
            <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="px-5 pt-4 shrink-0">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <DropZone eventType="Gol" color={eventAccent("Gol")} onFiles={addFiles} disabled={uploading} />
                        <DropZone eventType="Jogada" color={eventAccent("Jogada")} onFiles={addFiles} disabled={uploading} />
                    </div>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-2 text-center">
                        MP4, MOV, WebM, AVI · max 500 MB por arquivo
                    </p>
                </div>
                        ...
                        {sections.map((section) => section.entries.length > 0 && (
                            <div key={section.label}>
                                <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">
                                    <span className={`w-2 h-2 rounded-full ${section.accent} inline-block`} />
