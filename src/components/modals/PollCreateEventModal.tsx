            ...
            <div className="w-full sm:max-w-md max-h-[90vh] rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="px-5 py-4 border-b flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Crie um evento com confirmação de presença</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-1">Horário <span className="font-normal text-slate-400">(opcional)</span></label>
                            <input
                                type="time"
                                className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
                                value={eventTime}
                    ...
                    {/* Icon picker */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Ícone <span className="font-normal text-slate-400">(opcional)</span></label>
                        <div className="flex flex-wrap gap-1.5">
                            {EVENT_ICONS_CLOSE.map(ic => (
                                <button
                                    ...
                                    className={`h-8 w-8 rounded-lg text-lg flex items-center justify-center transition-colors border ${eventIcon === ic ? 'border-slate-900 bg-slate-100' : 'border-transparent hover:bg-slate-100'}`}
                                >
                                    {ic}
                                </button>
                            ))}
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">
                            ...
                            Custo <span className="font-normal text-slate-400">(opcional)</span>
                        </label>
                        <div className="flex gap-2">
                            {(['', 'individual', 'group'] as const).map(ct => (
                                <button
                                    ...
                                    className={`flex-1 rounded-lg border py-1.5 text-xs font-medium transition-colors ${costType === ct ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}
                                >
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">
                            ...
                            Prazo de resposta <span className="font-normal text-slate-400">(opcional)</span>
                        </label>
                        <div className="grid grid-cols-2 gap-2">
                            <input
                                type="date"
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                        <label className="flex items-center justify-between gap-3 cursor-pointer">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 mt-0.5">Mostrar quem confirmou/recusou</p>
                            </div>
                            <div
                ...
                <div className="px-5 py-3 border-t flex gap-2 justify-end shrink-0">
                    ...
                    <button
                        ...
                        type="button"
                        className="px-4 py-2 rounded-lg bg-slate-900 text-white text-sm font-semibold hover:bg-slate-700 disabled:opacity-50 flex items-center gap-2"
                    >
